...
public :
  ...
  TH1F *h_tauFakeEleSF = (TH1F*)f_tauFakeEleSF->Get("VLoose");
  ...
  TFile *f_muIDSF= new TFile("sf_files/RootFiles/muon/2018_RunABCD_SF_ID.root");
  TH2F *h_muIDSF=(TH2F*) f_muIDSF->Get("NUM_MediumID_DEN_TrackerMuons_pt_abseta");
  
  TFile *f_muIsoSF= new TFile("sf_files/RootFiles/muon/2018_RunABCD_SF_ISO.root");
  TH2F *h_muIsoSF=(TH2F*) f_muIsoSF->Get("NUM_TightRelIso_DEN_MediumID_pt_abseta");

  TFile *f_muTrgSF= new TFile("sf_files/RootFiles/muon/EfficienciesStudies_2018_trigger_EfficienciesAndSF_2018Data_AfterMuonHLTUpdate.root");
  TH2F *h_muTrgSF=(TH2F*) f_muTrgSF->Get("IsoMu24_PtEtaBins/pt_abseta_ratio");
  ...
  Int_t           run;
   ...
   double get_BinContent(TH2* histo, double xValue, double yValue);
   // bool hasGenTau();
...
void mutau_analyzer::Init(TChain *tree, string _isMC_, string sampleName)
{
  ...
  IsoMu24or27SF.init_ScaleFactors("sf_files/LeptonEfficiencies/Muon/Run2018/Muon_Run2018_IsoMu24orIsoMu27.root");
  CrossTriggerSF.init_ScaleFactors("sf_files/LeptonEfficiencies/Muon/Run2018/Muon_Run2018_IsoMu20.root");
  MuonIDIso.init_ScaleFactors("sf_files/LeptonEfficiencies/Muon/Run2018/Muon_Run2018_IdIso.root");
    //cout<<"from Init "<<isMC<<endl;
   // Set object pointer
   phoE = 0;
   phoEt = 0;
...
void mutau_analyzer::setMyEleTau(int muIndex, int tauIndex){
  ...
  if(is_MC){
    if (my_genmatching_l2>=5 && tau_DecayMode->at(tauIndex)==0) my_tauP4=my_tauP4*0.987;
    else if (my_genmatching_l2>=5 && tau_DecayMode->at(tauIndex)==1) my_tauP4=my_tauP4*0.995;
    else if (my_genmatching_l2>=5 && tau_DecayMode->at(tauIndex)==10) my_tauP4=my_tauP4*0.988;
    if ( (my_genmatching_l2==1 || my_genmatching_l2==3) && tau_DecayMode->at(tauIndex)==0) my_tauP4=my_tauP4*0.968;
    else if ( (my_genmatching_l2==1 || my_genmatching_l2==3) && tau_DecayMode->at(tauIndex)==1) my_tauP4=my_tauP4*1.026;
    if ( (my_genmatching_l2==2 || my_genmatching_l2==4) && tau_DecayMode->at(tauIndex)==0) my_tauP4=my_tauP4*0.998;
    else if ( (my_genmatching_l2==2 || my_genmatching_l2==4) && tau_DecayMode->at(tauIndex)==1) my_tauP4=my_tauP4*0.990;
  }
