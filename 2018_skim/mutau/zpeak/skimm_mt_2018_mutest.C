...
//
#define skimm_mt_2018_mutest_cxx
#include "skimm_mt_2018_mutest.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  skimm_mt_2018_mutest t(argv[1],argv[2], isMC);
...
void skimm_mt_2018_mutest::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
...
void skimm_mt_2018_mutest::BookHistos(const char* file2)
{
...
void skimm_mt_2018_mutest::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
bool skimm_mt_2018_mutest::skimming_Htt(){
  bool tmpCand=false;
  return tmpCand;
}

std::vector<int> skimm_mt_2018_mutest::skimmed_Mu(){
   ...
   for(int iMu=0;iMu<nMu;iMu++)
     {
       ...
       bool muonIso =  false;
       bool trigger = false;
       if( muPt->at(iMu) > 20  && fabs(muEta->at(iMu))<2.4  && fabs(muDz->at(iMu)) < 0.2 && fabs(muD0->at(iMu))<0.045 ) kinematic = true;
...
std::vector<int> skimm_mt_2018_mutest::skimmed_Tau(){
  std::vector<int> tmpCand;
  tmpCand.clear();
  return tmpCand;
...
void skimm_mt_2018_mutest::fillOutTree(){
  run_ = run;
  event_= event;
  lumis_=lumis;
  tree->Fill();
...
double skimm_mt_2018_mutest::dR(int mu_index, int tau_index)
{
  double deltaRtmp=0.0;
  return deltaRtmp;  
}

double skimm_mt_2018_mutest::DeltaPhi(double phi1, double phi2)
