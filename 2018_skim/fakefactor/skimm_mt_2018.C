...
//
#define skimm_mt_2018_cxx
#include "skimm_mt_2018.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  skimm_mt_2018 t(argv[1],argv[2], isMC);
...
void skimm_mt_2018::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
...
void skimm_mt_2018::BookHistos(const char* file2)
{
...
void skimm_mt_2018::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
std::vector<int> skimm_mt_2018::getMuCand_1(double muPtCut, double muEtaCut){
...
std::vector<int> skimm_mt_2018::getMuCand_2(double muPtCut, double muEtaCut, int skip_i){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over muons
  for(int iMu=0;iMu<nMu;iMu++)
...
std::vector<int> skimm_mt_2018::getTauCand(double tauPtCut, double tauEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
  for(int iTau=0;iTau<nTau;iTau++)
    {
      ...
      if( tau_byTightDeepTau2017v2p1VSmu->at(iTau) == 1 && tau_byVLooseDeepTau2017v2p1VSe->at(iTau)==1) tauId = true;
...
bool skimm_mt_2018::thirdLeptonVeto(int muon_1, int muon_2)
{
  //  std::vector<int> tmpCand;
  //tmpCand.clear();
  int tmpCand = 0;
...
double skimm_mt_2018::dR(int mu_1_index, int mu_2_index )
{
...
double skimm_mt_2018::dR_muTau(int mu_2_index, int tau_index )
{
...
double skimm_mt_2018::DeltaPhi(double phi1, double phi2)
...
float skimm_mt_2018::TMass_F(float LepPt, float LepPhi , float met, float metPhi) {
    return sqrt(pow(LepPt + met, 2) - pow(LepPt* cos(LepPhi) + met * cos(metPhi), 2) - pow(LepPt * sin(LepPhi) + met * sin(metPhi), 2));
}

float skimm_mt_2018::TotTMass_F(TLorentzVector a, TLorentzVector b, TLorentzVector met) {
  float totalTMass = (a + b+ met).M();
  return totalTMass;
}


float skimm_mt_2018::VisMass_F(TLorentzVector a, TLorentzVector b){
  float visibleMass = (a + b).M();
  return visibleMass;
}

float skimm_mt_2018::pTvecsum_F(float pt1, float pt2, float phi1, float phi2) {
  float pt_vecSum = sqrt( pow(pt1*cos(phi1) + pt2*cos(phi2), 2) + pow(pt1*sin(phi1) + pt2*sin(phi2), 2));
  return pt_vecSum;
}

bool skimm_mt_2018::passBjetVeto()
{
  int tmpCand = 0;
  bool veto = true;
