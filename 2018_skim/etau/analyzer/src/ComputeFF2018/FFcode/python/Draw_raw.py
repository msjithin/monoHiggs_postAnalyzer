...
def Draw_raw(step,year,channel):
    ...
    for i in range (0,ncat):
        ...
        VV=file.Get(categories[i]).Get("VVLT")
        VV.Add(file.Get(categories[i]).Get("STLT"))
        DY=file.Get(categories[i]).Get("DYLT")
        TTJ=file.Get(categories[i]).Get("TTJ")
        VVJ=file.Get(categories[i]).Get("VVJ")
        VVJ.Add(file.Get(categories[i]).Get("STJ"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--channel',defaults="et",help="")
    options = parser.parse_args()
    Draw_raw(options.step,options.year,options.channel)
