...
void etau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
  ...
  if ( sample.Contains("WJetsToLNu") ||
   ...
   double elePtCut=25.0; double tauPtCut=30.0;
   double eleEtaCut=2.1; double tauEtaCut=2.3;
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
     {
       ...
       if( ( //(HLTEleMuX>>3&1 == 1 )   //HLT_Ele27_WPTight_Gsf_v (HLTEleMuX>>3&1 == 1 )
	      (HLTEleMuX>>5&1 == 1)  //HLT_Ele32_WPTight_Gsf_v
	      || (HLTEleMuX>>61&1 == 1)   //HLT_Ele35_WPTight_Gsf_v
	     ))
       	 passSingleTriggerPaths=true;  //
       
       // if(run<317509)  ///// for cross trigger HLT_Ele24_eta2p1_WPTight_Gsf_LooseChargedIsoPFTau30_eta2p1_CrossL1
       // 	 {
       // 	   if(isMC==false &&  HLTTau>>1&1 == 1) 
       // 	     passCrossTrigger=true;
       // 	   else if (isMC==true &&  HLTTau>>1&1 == 1)
       ...
       // 	     passCrossTrigger=true;
       // 	 }
       // if( (HLTTau>>1&1 == 1 && run<317509 )
       // 	   || (HLTTau>>16&1 == 1 && run>=317509)
       // 	   )
       // 	 passCrossTrigger=true;
       if( (HLTTau>>1&1 == 1  )
       	   || (HLTTau>>16&1 == 1 )
       ...
       //if(eleEle_selector)
	 {
	   if(metFilters==0 )
	       ...
	       if(   passSingleTriggerPaths || passCrossTrigger )
		 {
		   ...
		   eleCand = getEleCand(elePtCut, eleEtaCut);  ///// ele selected
		   if( eleCand.size() >0 ) 
		     { 
		       ...
		       tauCand = getTauCand(tauPtCut, tauEtaCut);
		       if( tauCand.size()>0  ) 
			 {
				       ...
				       if( thirdLeptonVeto(reco_ele , reco_tau)  )
					 {
					   ...
					   if( passBjetVeto(reco_ele , reco_tau) )
					       ...
					       if(deltaR > 0.5 )
						 {
						   ...
						   double mT_eleMet = TMass_F( dau1.Pt(), dau1.Phi(),pfMET,pfMETPhi  );
						   if( mT_eleMet < 50 )
						     {
	       ...
	       if(  passSingleTriggerPaths || passCrossTrigger  )
		 {
		   ...
		   if( eleCand.size() >0 ) 
		     { 
		       ...
		       tauCand = getAISRTauCand(tauPtCut, tauEtaCut);
		       if( tauCand.size()>0 ) 
			 {
  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
	 ...
	 ( HLTEleMuX>>61&1 == 1 && dau1.Pt() > 33.0  )
	  ...
	  || ( HLTTau>>1&1 == 1 && dau1.Pt() > 25.0  && dau1.Pt() < 33.0 && fabs(dau1.Eta())< 2.1 )
	  || ( HLTTau>>16&1 == 1 && dau1.Pt() > 25.0  && dau1.Pt() < 33.0 && fabs(dau1.Eta())< 2.1 )
	  ) trigger = true;
  ...
  for(int iTau=0;iTau<nTau;iTau++)
    {
	 ...
	 ( HLTEleMuX>>61&1 == 1 )
	  ...
	  || ( HLTTau>>16&1 == 1 && tau_Pt->at(iTau) >35.0 && abs(tau_Eta->at(iTau)) < 2.1 )
	  ) trigger=true;
  ...
  for(int iTau=0;iTau<nTau;iTau++) //Loop over taus
    {
	 ...
	 ( HLTEleMuX>>61&1 == 1 )
	  ...
	  || ( HLTTau>>16&1 == 1 && tau_Pt->at(iTau) >35 && abs(tau_Eta->at(iTau)) < 2.1 )
          ) trigger=true;      
      if( kinematic==true    
	  && decayModeCut==true   
	  && tauIsolation==true 
...
double etau_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm, int tauGenMatch)
{
  bool debug=false;
  double rv_sf=1.0;
  ...
  if(tauGenMatch==1 || tauGenMatch==3) /// electrons to pass Deep
    {
      if(taudm==0)
  	{
  	  if(abs(taueta) < 1.479 ) sf_fakeEle=1.09*1.21;
  	  if(abs(taueta) > 1.479 ) sf_fakeEle=0.80*0.92;
  	}
      if(taudm==1)
  	{
  	  if(abs(taueta) < 1.479 ) sf_fakeEle=0.85*1.18;
  	  if(abs(taueta) > 1.479 ) sf_fakeEle=0.49*1.04;
  	}
    }
  if(tauGenMatch==2 || tauGenMatch==4) /// muons to pass Deep
    {
      if(taudm==0)
  	{
  	  if(abs(taueta)>0.0 && abs(taueta)<0.4 ) sf_fakeMu=1.11* 1.09;
	  else if(abs(taueta)>0.4 && abs(taueta)<0.8 ) sf_fakeMu=1.05* 0.78;
          else if(abs(taueta)>0.8 && abs(taueta)<1.2 ) sf_fakeMu=1.18* 0.77;
          else if(abs(taueta)>1.2 && abs(taueta)<1.7 ) sf_fakeMu=1.06* 0.75;
          else if(abs(taueta)>1.7 && abs(taueta)<2.3 ) sf_fakeMu=1.79* 2.02;
  	}
      if(taudm==1)
  	{
  	  if(abs(taueta)>0.0 && abs(taueta)<2.3) sf_fakeMu=0.79* 0.55;
  	}
    }
  if(tauGenMatch==1 || tauGenMatch==3) /// electrons -> tau ES
    {
      if(taudm==0)
  	sf_fakeEleES = 1 - (3.2)/100;
      if(taudm==1)
	sf_fakeEleES = 1 + (2.6)/100;
    }
  ...
  if( ( HLTEleMuX>>61&1==1 || HLTEleMuX>>5&1==1) && elept>33 )
    e_trg_sf=e_trg_sf;
  else
    e_trg_sf=1.0;
  if((HLTTau>>1&1 ==1 || HLTTau>>16&1==1) && elept<33)
  ...
  rv_sf =  sf_tauidSF_m * sf_fakeEle * sf_htt_workspace;
...
void etau_analyzer::fillHist( string histNumber , int eleIndex, int tauIndex, float event_weight, bool isMC){
  ...
  plotFill("elePt_"+hNumber,  eleP4.Pt() , 76 , 24 , 100,  event_weight);
  ...
  plotFill("metPhi_"+hNumber, pfMETPhi , 30, -3.14, 3.14,  event_weight);

  double mT_eleMet = TMass_F( eleP4.Pt(), eleP4.Phi() ,pfMET,pfMETPhi  );
...
void etau_analyzer::fillHist( string histNumber , TLorentzVector eleP4, TLorentzVector tauP4, int eleIndex, int tauIndex, float event_weight, bool isMC){
  string hNumber = histNumber;
  plotFill("elePt_"+hNumber,  elePt->at(eleIndex) , 76 , 24 , 100,  event_weight);
  plotFill("eleEta_"+hNumber, eleEta->at(eleIndex), 25, -2.5, 2.5,  event_weight);
  plotFill("elePhi_"+hNumber, elePhi->at(eleIndex), 30, -3.14, 3.14,  event_weight);
