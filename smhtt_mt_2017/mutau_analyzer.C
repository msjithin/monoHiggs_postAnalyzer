      ...
      if(metFilters==0 )
	{
	  ...
	  if(   passSingleTriggerPaths || passCrossTrigger )
		...
		{ 
		  ...
		  makeMyPlot("c", muCand[0],-1,event_weight);
		  ...
		  if( tauCand.size()>0 ) 
		    {
		      ...
		      makeMyPlot("d", MuIndex,TauIndex,event_weight); 
		      ...
		      if ( TriggerSelection(my_muP4, my_tauP4) )
			{
			  ...
			  if(pass3rdLeptonVeto)
				...
				//&&  (if_DY_Genmatching(MuIndex, TauIndex)==1 || if_DY_Genmatching(MuIndex, TauIndex)==3)  )
				{
				  ...
				  makeMyPlot("e", MuIndex,TauIndex,event_weight);
				      ...
				      if( thirdLeptonVeto(MuIndex , TauIndex)  )
					{
					  ...
					  if( pass_bjet_veto )
					      ...
					      if(deltaR > 0.5 )
						{
						  ...
						  makeMyPlot("i", MuIndex,TauIndex,event_weight);
						  ...
						  if( mT_muMet < 50 )
						    {
						      ...
						      makeMyPlot("j", MuIndex,TauIndex,event_weight);
						    }
						}
					    }
					}
...
double mutau_analyzer::getScaleFactors(  double mupt, double taupt, double mueta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  ...
  if(taupt<32.0) sf_tauTrg=1.0;
  double sf_htt_workspace=  m_tracking * m_IDiso * mu_singletrg * mu_crosstrg ;
  //rv_sf = sf_tauidSF_m * sf_tauTrg * sf_fakeEle * sf_fakeMu  * sf_muID*sf_IsoEff * sf_htt_workspace * higgPt_weight * weight_btagSF * top_pt_weight;
  rv_sf = sf_tauidSF_m * sf_tauTrg * sf_fakeEle * sf_fakeMu * sf_htt_workspace * higgPt_weight ;
  
  // printTabSeparated("rv_sf", rv_sf, "mupt", mupt, "taupt", taupt, "taudm", taudm, "tauGenMatch", tauGenMatch);
  // printTabSeparated(sf_tauidSF_m,sf_tauTrg,sf_fakeEle,sf_fakeMu,sf_htt_workspace,higgPt_weight,weight_btagSF,top_pt_weight);
  // if(mupt<28)
  //   printTabSeparated(m_tracking,m_IDiso,mu_singletrg,mu_crosstrg,zptmass_weight);
  // cout<<"\n\n"<<endl;
...
bool mutau_analyzer::TriggerSelection(TLorentzVector muP4, TLorentzVector tauP4){
  ...
  if( muP4.Pt() > 21.0 && muP4.Pt() <= 25.0 && tauP4.Pt()>32.0  && fabs(muP4.Eta())< 2.1  && fabs(tauP4.Eta())< 2.1 ){
    ...
    if(  ( HLTEleMuX>>21&1 == 1 && muP4.Pt() > 28.0 && tauP4.Pt()>30.0 )
	 || ( HLTEleMuX>>60&1 == 1 && muP4.Pt() > 25.0 && tauP4.Pt()>30.0 )
	 )
      return true;
  }
  else
...
void mutau_analyzer::makeMyPlot( string histNumber , int eleIndex, int tauIndex, float event_weight){
  string hNumber = histNumber;
  int ele_select=-1;
  std::vector<int> tmpCand; tmpCand.clear();
  if(eleIndex>-1)
  ...
  if(tauIndex>-1)
    ...
    tauIndex=0;
  plotFill("muPt_"+hNumber,  my_muP4.Pt() , 30 , 20 , 80,  event_weight);
  plotFill("muEta_"+hNumber, my_muP4.Eta(), 48, -2.4, 2.4,  event_weight);
  plotFill("muPhi_"+hNumber, my_muP4.Phi(), 30, -3.14, 3.14,  event_weight);
  ...
  int triggerBin1, triggerBin2, triggerBin3, triggerBin4;
  triggerBin1=triggerBin2=triggerBin3=triggerBin4=0;
  if( HLTEleMuX>>21&1 == 1 )  triggerBin4=4;
  if( HLTEleMuX>>60&1 == 1 ) triggerBin3=3;
  if( HLTTau>>0&1 == 1 )     triggerBin1=1;
  if(triggerBin1>0)
    plotFill("trigger_"+hNumber, triggerBin1 , 5, 0, 5,  event_weight);
  if(triggerBin2>0)
  ...
  plotFill("mass1_"+hNumber, my_muP4.M() , 50, 0, 0.05,  event_weight);
  plotFill("mass2_"+hNumber, my_tauP4.M() , 25, 0, 2.5,  event_weight);
  
  plotFill("genmatch1_"+hNumber, my_genmatching_l1 , 7, 0, 7,  event_weight);
  plotFill("genmatch2_"+hNumber, my_genmatching_l2 , 7, 0, 7,  event_weight);

  double deltaPhi = DeltaPhi(my_muP4.Phi(), my_tauP4.Phi());
  double deltaEta = fabs(my_muP4.Eta() - my_tauP4.Eta());
  plotFill("deltaPhi_"+hNumber, deltaPhi , 30, -3.14, 3.14,  event_weight);
  plotFill("deltaEta_"+hNumber, deltaEta ,  25, -2.5, 2.5,  event_weight);
  double deltaR = delta_R(my_muP4.Phi(), my_muP4.Eta(), my_tauP4.Phi(), my_tauP4.Eta()   );
  ...
  plotFill("metPhi_"+hNumber, my_metP4.Phi() , 30, -3.14, 3.14,  event_weight);
  double mT_eleMet = TMass_F( my_muP4.Pt(),my_muP4.Phi(),my_metP4.Pt(), my_metP4.Phi()  );
  ...
  float mt=TMass_F(my_muP4.Pt(),my_muP4.Phi(), my_metP4.Pt(), my_metP4.Phi());
  float mvis=VisMass_F(my_muP4, my_tauP4);
