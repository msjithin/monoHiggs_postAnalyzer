...
void smhet_2018::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //if( selectTrgEle32 || selectTrgEle35 || selectTrgCross)
	{
	  ...
	  //select electrons
	  if(pt_1>25 && fabs(eta_1)<2.1 && eid90_noiso_1 && iso_1<0.15)
	    {
	      makeMyPlot("c", 0, 0, 0, eventWeight);
	      // selct taus
	      if(pt_2>30 && fabs(eta_1)<2.3 && byMediumDeepVSjet_2 >0.5
		 && byTightDeepVSe_2 && byVLooseDeepVSmu_2 
