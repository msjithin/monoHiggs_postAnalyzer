  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
      ...
      bool trigger = false;
      if( ( HLTEleMuX>>61&1 == 1 && myEle.Pt() > 33.0  )
	  || ( HLTEleMuX>>5&1 == 1 && myEle.Pt() > 36.0 )
	  || ( HLTTau>>1&1 == 1 && myEle.Pt() > 25.0  && myEle.Pt() < 33.0 && fabs(myEle.Eta())< 2.1 )
	  || ( HLTTau>>16&1 == 1 && myEle.Pt() > 25.0  && myEle.Pt() < 33.0 && fabs(myEle.Eta())< 2.1 )
	  ) trigger = true;
  ...
  for(int iTau=0;iTau<nTau;iTau++)
    {
      ...
      if(is_MC){
	if (myGenMaching(iTau)>=5 && tau_DecayMode->at(iTau)==0) tauP4=tauP4*0.987;
	else if (myGenMaching(iTau)>=5 && tau_DecayMode->at(iTau)==1) tauP4=tauP4*0.995;
	else if (myGenMaching(iTau)>=5 && tau_DecayMode->at(iTau)==10) tauP4=tauP4*0.988;
	if (  (myGenMaching(iTau)==1 || myGenMaching(iTau)==3) && tau_DecayMode->at(iTau)==0 ) tauP4=tauP4*0.968;
	else if ( (myGenMaching(iTau)==1 || myGenMaching(iTau)==3) && tau_DecayMode->at(iTau)==1) tauP4=tauP4*1.026;
	if (  (myGenMaching(iTau)==2 || myGenMaching(iTau)==4) && tau_DecayMode->at(iTau)==0 ) tauP4=tauP4*0.998;
	else if ( (myGenMaching(iTau)==2 || myGenMaching(iTau)==4) && tau_DecayMode->at(iTau)==1) tauP4=tauP4*0.990;
      ...
      if(  ( HLTEleMuX>>61&1 == 1 )
	   ...
	   || ( HLTTau>>16&1 == 1 && tauP4.Pt() >35.0 && fabs(tauP4.Eta()) < 2.1 )
	  ) trigger=true;
  ...
  for(int iTau=0;iTau<nTau;iTau++) //Loop over taus
	...
	if (  (myGenMaching(iTau)==2 || myGenMaching(iTau)==4) && tau_DecayMode->at(iTau)==0 ) tauP4=tauP4*0.998;
      ...
      if( ( HLTEleMuX>>61&1 == 1 )
          ...
          || ( HLTTau>>16&1 == 1 && tauP4.Pt() >35 && fabs( tauP4.Eta()) < 2.1 )
	  ) trigger=true;      
      if( kinematic==true    
	  && decayModeCut==true   
	  && tauIsolation==true 
...
void etau_analyzer::makeMyPlot( string histNumber , int eleIndex, int tauIndex, float event_weight){
  ...
  if( HLTTau>>16&1 == 1 )     triggerBin2=2;
...
void etau_analyzer::applyESCorrections(TLorentzVector eleP4, TLorentzVector tauP4, int eleIndex, int tauIndex, TLorentzVector& eleP4Corr, TLorentzVector& tauP4Corr)
{
  ...
  if(is_MC){
    ...
    if (  (myGenMaching(tauIndex)==1 || myGenMaching(tauIndex)==3) && tau_DecayMode->at(tauIndex)==0 ) tauP4Corr=tauP4*0.968;
    else if ( (myGenMaching(tauIndex)==1 || myGenMaching(tauIndex)==3) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4*1.026;
    if (  (myGenMaching(tauIndex)==2 || myGenMaching(tauIndex)==4) && tau_DecayMode->at(tauIndex)==0 ) tauP4Corr=tauP4*0.998;
    else if ( (myGenMaching(tauIndex)==2 || myGenMaching(tauIndex)==4) && tau_DecayMode->at(tauIndex)==1) tauP4Corr=tauP4*0.990;
    
  }
  
}

int etau_analyzer::eventCategory(int eleIndex, int tauIndex){
  int category=0;
  std::vector<int> jetCand;       jetCand.clear();
