  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
      ...
      bool trigger = false;
      if (( HLTEleMuX>>61&1 == 1 && elePt->at(iEle) > 33.0  )
	  || ( HLTEleMuX>>5&1 == 1 && elePt->at(iEle) > 36.0 )
	  || ( HLTTau>>1&1 == 1 && elePt->at(iEle) > 25.0  && elePt->at(iEle) < 33.0 && fabs(eleEta->at(iEle))< 2.1 )
	  || ( HLTTau>>16&1 == 1 && elePt->at(iEle) > 25.0  && elePt->at(iEle) < 33.0 && fabs(eleEta->at(iEle))< 2.1 )
  ...
  for(int iTau=0;iTau<nTau;iTau++)
    {
	  ...
	  || ( HLTTau>>16&1 == 1 && tau_Pt->at(iTau) >35.0 && fabs(tau_Eta->at(iTau)) < 2.1 )
	  ) trigger=true;
