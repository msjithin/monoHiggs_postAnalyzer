#define smhet_2018_cxx
#include "smhet_2018.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  smhet_2018 t(argv[1],argv[2], isMC);
...
void smhet_2018::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
   if (fChain == 0) return;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     Long64_t ientry = LoadTree(jentry);
      ...
      bool selectTrgCrossHPS=(passEle24HPSTau30 
			      && pt_1>25 && pt_1<33 && pt_2>35 && abs(eta_1)<2.1 && abs(eta_2)<2.1
      ...
      if( selectTrgEle32 || selectTrgEle35 || selectTrgCross || selectTrgCrossHPS)
...
void smhet_2018::BookHistos(const char* file1, const char* file2)
{
...
double smhet_2018::delta_R(float phi1, float eta1, float phi2, float eta2)
{
...
float smhet_2018::pTvecsum_F(TLorentzVector a, TLorentzVector b, TLorentzVector c) {
  float pt_vecSum = (a + b+ c).Pt();
  return pt_vecSum;
}
double smhet_2018::DeltaPhi(double phi1, double phi2)
...
float smhet_2018::TMass_F(float LepPt, float LepPhi , float met, float metPhi) {
...
void smhet_2018::makeMyPlot( string histNumber , int eleIndex, int ele2Index, int tauIndex, float event_weight){
...
double smhet_2018::getScaleFactors( double elept, double taupt, double eleeta, double taueta, int taudm)
{
  bool debug=false;
  double rv_sf=1.0;
