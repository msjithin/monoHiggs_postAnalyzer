...
double mutau_analyzer::getScaleFactors(double mupt, double taupt, double mueta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  ...
  double sf_fakeEle = 1.0;
  double sf_fakeMu = 1.0;
  ...
  { ///  muons to pass deep tau
    if (abs(taueta) < 0.4)
      sf_fakeMu = 1.17;
    else if (abs(taueta) > 0.4 && abs(taueta) < 0.8)
      sf_fakeMu = 1.29;
    else if (abs(taueta) > 0.8 && abs(taueta) < 1.2)
      sf_fakeMu = 1.14;
    else if (abs(taueta) > 1.2 && abs(taueta) < 1.7)
      sf_fakeMu = 0.93;
    else if (abs(taueta) > 1.7 && abs(taueta) < 2.3)
      sf_fakeMu = 1.61;
  }
  ...
  if (taudm == 0)
    sf_tauTrg = h_tauTrgSF_dm0->GetBinContent(h_tauTrgSF_dm0->GetXaxis()->FindBin(tauPtCheck));
  if (taudm == 1)
    sf_tauTrg = h_tauTrgSF_dm1->GetBinContent(h_tauTrgSF_dm1->GetXaxis()->FindBin(tauPtCheck));
  if (taudm == 10)
    sf_tauTrg = h_tauTrgSF_dm10->GetBinContent(h_tauTrgSF_dm10->GetXaxis()->FindBin(tauPtCheck));
  if (taudm == 11)
    sf_tauTrg = h_tauTrgSF_dm11->GetBinContent(h_tauTrgSF_dm11->GetXaxis()->FindBin(tauPtCheck));
  ...
  {
    int t_index = get_t_Cand();
    int tbar_index = get_tbar_Cand();
    if (t_index > -1 && tbar_index > -1)
    {
      double pttop1 = mcPt->at(t_index);
      double pttop2 = mcPt->at(tbar_index);
      if (pttop1 > 400)
        pttop1 = 400;
      if (pttop2 > 400)
        pttop2 = 400;
