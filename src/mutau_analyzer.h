...
public:
  ...
  virtual vector<int> simple_boostedtau_cand(double pt, double eta, int shift);
  ...
  vector<int> bJet_medium();
  vector<int> bJet_loose();
  ...
  void setMyEleTau(int eleIndex, int tauIndex, TLorentzVector metP4, int shift, bool isBoostedtau);
  void save_nom();
  ...
  void get_index(vector<int> mu_indices, vector<int> tau_indices, vector<int> boostedtau_indices, int &mu_index, int &tau_index, bool &isBoosted);
  void get_index_jettau(vector<int> mu_indices, vector<int> tau_indices, vector<int> boostedtau_indices, int &mu_index, int &tau_index, bool &isBoosted);
};
...
void mutau_analyzer::Init(TChain *tree, string _isMC_, string sampleName)
{
  ...
  lheNormalizedWeights = 0;
   ...
   boostedTauByIsolationMVArun2v1DBnewDMwLTrawNew = 0;
   boostedTauByVLooseIsolationMVArun2v1DBnewDMwLTNew = 0;
   boostedTauByLooseIsolationMVArun2v1DBnewDMwLTNew = 0;
   boostedTauByMediumIsolationMVArun2v1DBnewDMwLTNew = 0;
   boostedTauByTightIsolationMVArun2v1DBnewDMwLTNew = 0;
   boostedTauByVTightIsolationMVArun2v1DBnewDMwLTNew = 0;
   boostedTauEta = 0;
   boostedTauPhi = 0;
   boostedTauPt = 0;
   boostedTauEt = 0;
...
void mutau_analyzer::setMyEleTau(int muIndex, int tauIndex, TLorentzVector event_metP4, int shift, bool isBoostedtau)
{
  ...
  if(isBoostedtau)
  {
    my_tauP4.SetPtEtaPhiE(boostedTauPt->at(tauIndex), boostedTauEta->at(tauIndex), boostedTauPhi->at(tauIndex), boostedTauEnergy->at(tauIndex));
    tauIsolation = boostedTauByMediumIsolationMVArun2v1DBoldDMwLTNew->at(tauIndex);
    tauDecayMode = boostedTauDecayMode->at(tauIndex);
    taucharge = boostedTauCharge->at(tauIndex);
    tauAntiEle = boostedTauByMVA6LooseElectronRejection->at(tauIndex);
    tauAntiMu = boostedTauByLooseMuonRejection3->at(tauIndex);
  }
  else
  {
    my_tauP4.SetPtEtaPhiE(tau_Pt->at(tauIndex), tau_Eta->at(tauIndex), tau_Phi->at(tauIndex), tau_Energy->at(tauIndex));
    tauIsolation = tau_byMediumDeepTau2017v2p1VSjet->at(tauIndex);
    tauDecayMode = tau_DecayMode->at(tauIndex);
    taucharge = tau_Charge->at(tauIndex);
    tauAntiEle = tau_byTightDeepTau2017v2p1VSe->at(tauIndex);
    tauAntiMu = tau_byVLooseDeepTau2017v2p1VSmu->at(tauIndex);
  }
    ...
    deltaPhi  = my_muP4.DeltaPhi(my_tauP4);
    deltaEta  = abs(my_muP4.Phi() - my_tauP4.Phi());
    mT_muMet  = TMass_F( my_muP4.Pt(),my_muP4.Phi(), my_metP4.Pt(), my_metP4.Phi() );
    visMass_mutau = (my_muP4+ my_tauP4).M();
    HiggsPt   = (my_muP4 + my_tauP4).Pt();
    delpta_phi_HptMet = (my_muP4+my_tauP4).DeltaPhi(my_metP4);
    tot_tr_mass  =  (my_muP4 + my_tauP4 + my_metP4 ).M();
