...
public:
  ...
  TFile *f_tauesSF_highpt = new TFile("sf_files/TauIDSFs/data/TauES_dm_DeepTau2017v2p1VSjet_2017ReReco.root");
  ...
  TFile *f_muIDSF = TFile::Open("sf_files/2017/RunBCDEF_SF_ID.root", "READ");
  ...
  TFile *f_muIsoSF = TFile::Open("sf_files/2017/RunBCDEF_SF_ISO.root", "READ");
  ...
  TFile *f_muTrgSF = new TFile("sf_files/2017/EfficienciesAndSF_RunBtoF_Nov17Nov2017.root");
  ...
  KITMuSF IsoMu24or27SF;
  KITMuSF CrossTriggerSF;
  KITMuSF MuonIDIso;
  TauIDSFTool tauSFTool{"2017ReReco", "DeepTau2017v2p1VSjet", "Medium"};
  MEtSys metSys;
  // IsoMu24or27SF.init_ScaleFactors("some file name here");
  ...
  std::vector<std::pair<string, int>> tauDM_map;
  TLorentzVector my_tauP4, my_muP4, my_metP4;
  TLorentzVector my_tauP4_nom_5, my_muP4_nom_5, my_metP4_nom_5;
  TLorentzVector my_tauP4_nom_9, my_muP4_nom_9, my_metP4_nom_9;
  TLorentzVector my_tauP4_nom_5dyll, my_muP4_nom_5dyll, my_metP4_nom_5dyll;
  TLorentzVector my_tauP4_nom_9dyll, my_muP4_nom_9dyll, my_metP4_nom_9dyll;
  std::vector<int> muCand, tauCand, aisrtauCand;
  std::vector<int> muCand_nom, tauCand_nom, aisrtauCand_nom;
  std::vector<int> jetCand;
  std::vector<int> jetCand_nom;
  vector<float> orginal_jetPt;

  string unc_shift;
  int MuIndex, TauIndex;
  int MuIndex_nom, TauIndex_nom, MuIndex_nomdyll, TauIndex_nomdyll;
  int my_njets, my_njets_nom_5, my_njets_nom_9, my_njets_nom_5dyll, my_njets_nom_9dyll;
  int my_genmatching_l1, l1_genmatching_nom, l1_genmatching_nomdyll;
  int my_genmatching_l2, l2_genmataching_nom, l2_genmataching_nomdyll;

  bool debug;
  bool check_unc;
  string stage;
  int shift_index;
  string selected_systematic;
  double applySf;
  bool passSingleTriggerPaths;
  bool passCrossTrigger;
  bool Ztt_selector;
  int eventNumber;
  ...
  mutau_analyzer(const char *file1, const char *file2, string isMC, string SampleName, const char *signal_para);
  ...
  void selections(float weight, int shift, string uncObject);
  ...
  virtual vector<int> simple_mu_cand(double pt, double eta, int shift);
  virtual vector<int> simple_tau_cand(double pt, double eta, int shift);
  virtual vector<int> getMuCand(double pt, double eta, int shift);
  ...
  int thirdLeptonVeto();
  bool thirdLeptonVeto(int muIndex, int tauIndex);
  bool thirdLeptonVeto(int muIndex, int tauIndex, int mu2Index);
  ...
  vector<int> bJet_medium(int muIndex, int tauIndex);
  vector<int> bJet_loose(int muIndex, int tauIndex);
  bool passBjetVeto_medium(int muIndex, int tauIndex);
  bool passBjetVeto_loose(int muIndex, int tauIndex);
  bool passBjetVeto(int muIndex, int tauIndex);
  ...
  void JESUnc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void JERUnc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void tauESunc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void tauTRGunc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void tauIDunc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void leptonTRGunc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void electronES(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void electronMissID(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void muonES(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void muonMissID(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void dyShape(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void ttbarShape(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void metSysUnc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void metClusteredUnc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  void prefiringUnc(string histNumber, int eleIndex, int tauIndex, bool isFakeBkg, float event_weight);
  ...
  bool MatchTriggerFilter(int muIndex, int tauIndex);
  bool TriggerSelection(TLorentzVector muP4, TLorentzVector tauP4);
  void makeTestPlot(string histNumber, int muIndex, int mu2Index, int tauIndex, float event_weight);
  bool passDiMuonVeto(int muIndex);
  bool eVetoZTTp001dxyz(int muIndex, int tauIndex);
  bool mVetoZTTp001dxyz(int muIndex, int tauIndex);
  TLorentzVector MetRecoilCorrections(int muIndex, int tauIndex, TLorentzVector mymet);
  ...
  pair<int, int> get_index(vector<int> mu_indices, vector<int> tau_indices);
  pair<int, int> get_index_2(vector<int> mu_indices, vector<int> tau_indices);
};

#endif

#ifdef mutau_analyzer_cxx
mutau_analyzer::mutau_analyzer(const char *file1, const char *file2, string isMC, string sampleName, const char *signal_para)
{
  ...
  if (found_ZprimeBaryonic)
    zprimeBaryonic_signal = atof(signal_para);
...
void mutau_analyzer::Init(TChain *tree, string _isMC_, string sampleName)
{
  ...
  tauDM_map.push_back({"3prong1pizero", 11});
...
void mutau_analyzer::setMyEleTau(int muIndex, int tauIndex, TLorentzVector event_metP4, int shift)
{
  ...
  /* cout<<"pfMet="<<pfMET<<"  pfMET_T1JESUp="<<pfMET_T1JESUp<<"    pfMET_T1JERUp="<<pfMET_T1JERUp<<endl; */
  /* cout<<"pfMet="<<pfMET<<"  pfMET_T1JESDo="<<pfMET_T1JESUp<<"    pfMET_T1JERDo="<<pfMET_T1JERUp<<endl; */
  /* cout<<"First in setmyle event "<<eventNumber<<"  unc_shift="<<unc_shift<<" sys="<<selected_systematic */
  /*     <<"  met ="<<metP4.Pt() */
  /*     <<"\n"<<endl; */

  TLorentzVector uncorrected_met;
  TLorentzVector uncorrectedMetPlusTau;
  ...
  if (passDiMuonVeto(MuIndex) == true && eVetoZTTp001dxyz(MuIndex, TauIndex) && mVetoZTTp001dxyz(MuIndex, TauIndex))
    Ztt_selector = true;
  else
    Ztt_selector = false;
...
void mutau_analyzer::save_nom()
{

  if (stage == "5_dyll")
  {
    MuIndex_nomdyll = MuIndex;
    TauIndex_nomdyll = TauIndex;
    l1_genmatching_nomdyll = my_genmatching_l1;
    l2_genmataching_nomdyll = my_genmatching_l2;
    my_tauP4_nom_5dyll = my_tauP4;
    my_muP4_nom_5dyll = my_muP4;
    my_metP4_nom_5dyll = my_metP4;
    my_njets_nom_5dyll = my_njets;
  }
  if (stage == "9_dyll")
  {
    MuIndex_nomdyll = MuIndex;
    ...
    l1_genmatching_nomdyll = my_genmatching_l1;
    l2_genmataching_nomdyll = my_genmatching_l2;
    my_tauP4_nom_9dyll = my_tauP4;
    my_muP4_nom_9dyll = my_muP4;
    my_metP4_nom_9dyll = my_metP4;
    my_njets_nom_9dyll = my_njets;
  }
  if (stage == "5")
  {
    MuIndex_nom = MuIndex;
    TauIndex_nom = TauIndex;
    l1_genmatching_nom = my_genmatching_l1;
    l2_genmataching_nom = my_genmatching_l2;
    my_tauP4_nom_5 = my_tauP4;
    my_muP4_nom_5 = my_muP4;
    my_metP4_nom_5 = my_metP4;
    my_njets_nom_5 = my_njets;
  }
  if (stage == "9")
  {
    MuIndex_nom = MuIndex;
    ...
    l1_genmatching_nom = my_genmatching_l1;
    l2_genmataching_nom = my_genmatching_l2;
    my_tauP4_nom_9 = my_tauP4;
    my_muP4_nom_9 = my_muP4;
    my_metP4_nom_9 = my_metP4;
    my_njets_nom_9 = my_njets;
  }
}

pair<int, int> mutau_analyzer::get_index(vector<int> mu_indices, vector<int> tau_indices)
{
  ...
  if (mu_indices.size() > 0 && tau_indices.size() > 0)
  {
    ...
    {
      if (mu_index >= 0 && tau_index >= 0) break;
      for (int j = 0; j < tau_indices.size(); j++)
      {
        ...
        relMuIso_old = (muPFChIso->at(iMu) + max(muPFNeuIso->at(iMu) + muPFPhoIso->at(iMu) - 0.5 * muPFPUIso->at(iMu), 0.0)) / (muPt->at(iMu));

        double diff_PFChIso  = muPFChIso->at(iMu)  - tau_chargedIsoPtSum->at(iTau);
        double diff_PFNeuIso = muPFNeuIso->at(iMu) - tau_neutralIsoPtSum->at(iTau);
        double diff_PFPUIso  = muPFPUIso->at(iMu)  - tau_puCorrPtSum->at(iTau);
        relMuIso_v3 = (diff_PFChIso + max(diff_PFNeuIso + muPFPhoIso->at(iMu) - 0.5 * diff_PFPUIso, 0.0)) / (muPt->at(iMu));

        double relMuIso_to_use = 0.0;
        ...
        if (mu_tau_dr < 0.7 ) 
          {
            relMuIso_to_use = relMuIso;
            is_boosted = true;
          }
        else  
        {
          relMuIso_to_use = relMuIso_old;
          is_boosted = false;
        }
        // relMuIso_to_use = relMuIso_old;
        if (make_iso_plot){
          plotFill("new_relMuIso_to_use", relMuIso_to_use, 200, -10, 10, 1.0);
          plotFill("ref_relMuIso_old", relMuIso_old, 200, -10, 10, 1.0);
          plotFill("ref_relMuIso_new", relMuIso, 200, -10, 10, 1.0);
          plotFill("new_relMuIso_v3", relMuIso_v3, 200, -10, 10, 1.0);
        }
        if (mu_tau_dr < 0.7 && muCharge->at(iMu) * tau_Charge->at(iTau) < 0)
        {
          plotFill("muon_relMuIso_v2", relMuIso, 200, -10, 10, 1.0);
          plotFill("muon_relMuIso_v3", relMuIso_v3, 200, -10, 10, 1.0);
        }
        else if (mu_tau_dr >= 0.7 && muCharge->at(iMu) * tau_Charge->at(iTau) < 0)
        {
          plotFill("muon_relMuIso_v2", relMuIso_old, 200, -10, 10, 1.0);
          plotFill("muon_relMuIso_v3", relMuIso_old, 200, -10, 10, 1.0);
        }
        if ( is_boosted==false
            && muCharge->at(iMu) * tau_Charge->at(iTau) < 0 
            && muIDbit->at(iMu) >> 1 & 1 == 1 // muon id
            && relMuIso_to_use < 0.15               // muon relatice isolation
            && tau_byMediumDeepTau2017v2p1VSjet->at(iTau)==1
            && tau_IDbits->at(iTau)>>1&1==1
            && ( tau_DecayMode->at(iTau)==0 || tau_DecayMode->at(iTau)==1 || tau_DecayMode->at(iTau)==10 || tau_DecayMode->at(iTau)==11 )
            && ( tau_byVLooseDeepTau2017v2p1VSe->at(iTau)==1 && tau_byTightDeepTau2017v2p1VSmu->at(iTau)==1)
        ...
        )
        {
          if (make_iso_plot)
          {
            plotFill("new_relMuIso_new", relMuIso, 200, -10, 10, 1.0);
            plotFill("new_relMuIso_old", relMuIso_old, 200, -10, 10, 1.0);
            plotFill("new_relMuIso_final_a", relMuIso, 200, -10, 10, 1.0);
        ...
        else if ( is_boosted==true
            && muCharge->at(iMu) * tau_Charge->at(iTau) < 0 
            && muIDbit->at(iMu) >> 1 & 1 == 1 // muon id
            && relMuIso_to_use < 0.15               // muon relatice isolation
            // && tau_byMediumDeepTau2017v2p1VSjet->at(iTau)==1
            // && tau_byVVVLooseDeepTau2017v2p1VSjet->at(iTau)==1
            && tau_IDbits->at(iTau)>>1&1==1
            && ( tau_DecayMode->at(iTau)==0 || tau_DecayMode->at(iTau)==1 || tau_DecayMode->at(iTau)==10 || tau_DecayMode->at(iTau)==11 )
            // && ( tau_byVLooseDeepTau2017v2p1VSe->at(iTau)==1 && tau_byTightDeepTau2017v2p1VSmu->at(iTau)==1)
            && (TriggerSelection(mu_p4, tau_p4) == true )
  ...
  return make_pair(mu_index, tau_index);
}
pair<int, int> mutau_analyzer::get_index_2(vector<int> mu_indices, vector<int> tau_indices)
{
  ...
  if (mu_indices.size() > 0 && tau_indices.size() > 0)
  {
    for (int i = 0; i < mu_indices.size(); i++)
    {
      ...
      for (int j = 0; j < tau_indices.size(); j++)
      {
        ...
        bool pass_bjet_veto = ((bJet_medium(iMu, iTau).size() == 0) && (bJet_loose(iMu, iTau).size() < 2));
        bool pass3rdLeptonVeto = (passDiMuonVeto(iMu) == true && eVetoZTTp001dxyz(iMu, iTau) && mVetoZTTp001dxyz(iMu, iTau));
        if (mu_tau_dr < 1.0 )//(higgspt>100) //(mu_tau_dr < 0.7 )
          {
            relMuIso_to_use = relMuIso;
            is_boosted = true;
          }
        ...
        if ( is_boosted==false
            && muCharge->at(iMu) * tau_Charge->at(iTau) < 0 
            && muIDbit->at(iMu) >> 1 & 1 == 1 // muon id
            && relMuIso_to_use < 0.3               // muon relatice isolation
            && (TriggerSelection(mu_p4, tau_p4) == true )
            && (thirdLeptonVeto(iMu, iTau))
            && (pass_bjet_veto)
            && (mu_tau_dr > 0.3)
