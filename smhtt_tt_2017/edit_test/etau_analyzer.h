...
void etau_analyzer::setMyEleTau(int eleIndex, int tauIndex){
  ...
  if(is_MC){
    ...
    if (  (myGenMaching(tauIndex)==1 || myGenMaching(tauIndex)==3) && tau_DecayMode->at(tauIndex)==0 ) my_tauP4=my_tauP4*1.003;
    else if ( (myGenMaching(tauIndex)==1 || myGenMaching(tauIndex)==3) && tau_DecayMode->at(tauIndex)==1) my_tauP4=my_tauP4*1.036;
  }
