  ...
  //sw.Start();
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
       ...
       if(metFilters==0)
	 {
	   ...
	   if(debug)cout<<"genweight applied "<<" line#"<<__LINE__<<endl;
	   if(  passCrossTrigger  )
	     {
	       nSingleTrgPassed+=event_weight;
	       if(debug)cout<<"trigger selected"<<" line#"<<__LINE__<<endl;
	       makeMyPlot("b", -1,-1,event_weight);
	       tau1Cand = getTauCand(40.0,2.1);  ///// ele selected 
	       if( tau1Cand.size() >0 ) 
		 { 
		   nGoodMuonPassed+=event_weight;
		   if(debug)cout<<"this worked "<<" line#"<<__LINE__<<endl;
		   //cout<<"Good Ele Passed"<<endl;
		   makeMyPlot("c", tau1Cand[0],-1,event_weight);
		   //tau2Cand = getTauCand(40.0,2.1);
		   if( tau1Cand.size() >=2 )
		     {
		       nGoodTauPassed+=event_weight;
		       if(debug)cout<<"Good Tau Passed"<<" line#"<<__LINE__<<endl;
		       if(debug)cout<<"tau1Cand size = "<<tau1Cand.size() <<endl;
		       setMyEleTau(tau1Cand[0], tau1Cand[1]);
		       //cout<<" : "<<my_tau1P4.Pt()<<" taupt:"<<   my_tau2P4.Pt() <<" line#"<<__LINE__<<endl;
		       makeMyPlot("d", Tau1Index,Tau2Index,event_weight);
		       ...
		       if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
		       if ( TriggerSelection(my_tau1P4, my_tau2P4) )
			 {
			   if(Ztt_selector) 
			       ...
			       if (  tau_Charge->at(Tau1Index) * tau_Charge->at(Tau2Index) < 0  ) 
				 {
				   ...
				   //cout<<"opp charge selection"<<endl;
				   if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
				   afterSF1+=event_weight;
				   makeMyPlot("e", Tau1Index, Tau2Index,event_weight);
				   if (  MatchTriggerFilter(Tau1Index, Tau2Index) )
				     {
				       ...
				       makeMyPlot("f", Tau1Index, Tau2Index,event_weight);
				       if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
				       if( thirdLeptonVeto(Tau1Index, Tau2Index)  )
					 {
					   nPassedThirdLepVeto+=event_weight;
					   makeMyPlot("g", Tau1Index, Tau2Index,event_weight);
					   if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
					   //if( pass_bjet_veto )
					   {
					     if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
					     nPassedBjetVeto+=event_weight;
					     makeMyPlot("h", Tau1Index, Tau2Index,event_weight);
						 ...
						 makeMyPlot("i", Tau1Index, Tau2Index,event_weight);
						 ...
						 if( mT_eleMet < 50 )
						   {
						     ...
						     if(debug)cout<<" Passed"<<" line#"<<__LINE__<<endl;
						     makeMyPlot("j", Tau1Index, Tau2Index,event_weight);
  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
      ...
      applyEleESCorrections(myEle, iEle, myEle);
      ...
      if( kinematic && relative_iso && electronId){
  ...
  for(int iTau=0;iTau<nTau;iTau++)
    {
      ...
      if(is_MC)
        applyTauESCorrections(tauP4, iTau, tauP4);
      bool kinematic = false;
      bool tauId = false;
...
double tautau_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm, int tauGenMatch, bool isFakebkg)
{
  bool debug=false;
  double rv_sf=1.0;
...
void tautau_analyzer::fillHist( string histNumber , int eleIndex, int tauIndex, bool isFakeBkg, float event_weight){
...
int tautau_analyzer::eventCategory(int eleIndex, int tauIndex, double higgsPt){
  int category=0;
  std::vector<int> jetCand;       jetCand.clear();
...
void tautau_analyzer::makeMyPlot( string histNumber , int tau1Index, int tau2Index, float event_weight){
  string hNumber = histNumber;
  int ele_select=-1;
  std::vector<int> tmpCand; tmpCand.clear();
  if(tau1Index>-1)
  ...
  if(tau2Index>-1)
    ...
    tau2Index=0;
  plotFill("tau1Pt_"+hNumber,  my_tau1P4.Pt() , 38 , 24 , 100,  event_weight);
  plotFill("tau1Eta_"+hNumber, my_tau1P4.Eta(), 25, -2.5, 2.5,  event_weight);
  plotFill("tau1Phi_"+hNumber, my_tau1P4.Phi(), 30, -3.14, 3.14,  event_weight);

  plotFill("tau2Pt_"+hNumber,  my_tau2P4.Pt()  , 25 , 30 , 80 ,  event_weight);
  plotFill("tau2Eta_"+hNumber, my_tau2P4.Eta(), 25, -2.5, 2.5,  event_weight);
  ...
  if(tau_IDbits->at(tau1Index)>>1&1==1) decayModeFinding_2=1;
  ...
  if( HLTTau>>5&1 == 1 )  triggerBin3=3;
  if( HLTTau>>6&1 == 1 )  triggerBin2=2;
  if( HLTTau>>7&1 == 1 )  triggerBin1=1;
  if(triggerBin1>0)
  ...
  plotFill("mass1_"+hNumber, my_tau1P4.M() , 50, 0, 0.05,  event_weight);
  plotFill("mass2_"+hNumber, my_tau2P4.M() , 25, 0, 2.5,  event_weight);
  
  plotFill("genmatch1_"+hNumber, myGenMaching(tau1Index) , 7, 0, 7,  event_weight);
  plotFill("genmatch2_"+hNumber, myGenMaching(tau2Index) , 7, 0, 7,  event_weight);
  ...
  //cout<<"     elePt_"<<hNumber<<" = "<< elePt->at(tmpCand[0])<<endl;
  plotFill("event_"+hNumber, event/1000000, 1000, 0.0, 1000, event_weight);
}
