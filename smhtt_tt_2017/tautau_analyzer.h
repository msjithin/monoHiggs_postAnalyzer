...
void tautau_analyzer::setMyEleTau(int tau1Index, int tau2Index){
  ...
  if(is_MC){
    if (my_genmatching_l2>=5 && tau_DecayMode->at(tau2Index)==0) my_tau2P4=my_tau2P4*1.007;
    else if (my_genmatching_l2>=5 && tau_DecayMode->at(tau2Index)==1) my_tau2P4=my_tau2P4*0.998;
    else if (my_genmatching_l2>=5 && tau_DecayMode->at(tau2Index)==10) my_tau2P4=my_tau2P4*1.001;
    if (  (my_genmatching_l2==1 || my_genmatching_l2==3) && tau_DecayMode->at(tau2Index)==0 )
      my_tau2P4=my_tau2P4*1.003;
    else if ( (my_genmatching_l2==1 || my_genmatching_l2==3) && tau_DecayMode->at(tau2Index)==1)
      my_tau2P4=my_tau2P4*1.036;
  }
  if(is_MC){
    if (my_genmatching_l1>=5 && tau_DecayMode->at(tau1Index)==0) my_tau1P4=my_tau1P4*1.007;
    else if (my_genmatching_l1>=5 && tau_DecayMode->at(tau1Index)==1) my_tau1P4=my_tau1P4*0.998;
    else if (my_genmatching_l1>=5 && tau_DecayMode->at(tau1Index)==10) my_tau1P4=my_tau1P4*1.001;
    if (  (my_genmatching_l1==1 || my_genmatching_l1==3) && tau_DecayMode->at(tau1Index)==0 )
      my_tau1P4=my_tau1P4*1.003;
    else if ( (my_genmatching_l1==1 || my_genmatching_l1==3) && tau_DecayMode->at(tau1Index)==1)
  ...
  //pass_bjet_veto = (bJet_medium(Tau1Index, Tau2Index).size()==0) && (bJet_loose(Tau1Index, Tau2Index).size()<2);
  //btag_sf=btag_sf_weight(Tau1Index , Tau2Index);
  //cout<<"setmyeletau finished "<<endl;
}
