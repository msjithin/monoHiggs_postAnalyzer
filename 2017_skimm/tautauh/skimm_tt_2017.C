...
//
#define skimm_tt_2017_cxx
#include "skimm_tt_2017.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  skimm_tt_2017 t(argv[1],argv[2], isMC);
...
void skimm_tt_2017::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
...
void skimm_tt_2017::BookHistos(const char* file2)
{
...
void skimm_tt_2017::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
bool skimm_tt_2017::skimming_Htt(){
  bool tmpCand=false;
  ...
  for(int iTau=0; iTau<nTau;iTau++){
    if(tmpTau1Cand.size()>0)
      if( iTau==tmpTau1Cand[0] )
	continue;
    if( tau_Pt->at(iTau) > 29.5
        && fabs( tau_Eta->at(iTau))< 2.1
        && (tau_DecayMode->at(iTau) !=5 && tau_DecayMode->at(iTau)!=6)
...
void skimm_tt_2017::fillOutTree(){
  run_ = run;
  event_= event;
  lumis_=lumis;
  tree->Fill();
...
double skimm_tt_2017::dR(int tau1_index, int tau2_index)
{
...
double skimm_tt_2017::DeltaPhi(double phi1, double phi2)
