...
#ifndef tautau_analyzer_h
#define tautau_analyzer_h
...
public :
  ...
  TFile * frawff = TFile::Open("sf_files/ComputeFF2018/ff_files_tt_2017/uncorrected_fakefactors_tt.root");
  ...
  //TFile *fmvisclosure = TFile::Open("sf_files/ComputeFF2018/ff_files_tt_2017/FF_corrections_1.root");
  //TF1* mvisclosure_qcd=(TF1*) fmvisclosure->Get("closure_mvis_tt_qcd");
  TF1* subleadingTauPt_correction = new TF1("subleadingTauPt_correction","-0.001083333*x+1.03",0,1000); 
  //TF1 *fa2 = new TF1("fa2","TMath::DiLog(x)",0,10);
  TFile *fosssclosure  = TFile::Open("sf_files/ComputeFF2018/ff_files_tt_2017/FF_QCDcorrectionOSSS_tt.root");
  TF1* osssclosure_qcd=(TF1*) fosssclosure->Get("closure_OSSS_mvis_tt_qcd");
  ...
  int my_njets;
  int my_genmatching_l2, tau1dm;
  int my_genmatching_l1, tau2dm;
  ...
  // Declaration of leaf types
   ...
   //   tautau_analyzer(TTree *tree=0);
   tautau_analyzer(const char* file1, const char* file2, string isMC, string SampleName);
   ~tautau_analyzer();
   ...
   vector<int> getTauCand_noID(double pt, double eta);
   vector<int> getJetCand(int eleIndex, int tauIndex);
   void assignTauIndices(vector<int> tauIndices , int& tau1 , int& tau2 );
   int getZCand();
   int get_t_Cand();
   int get_tbar_Cand();
...
#ifdef tautau_analyzer_cxx
tautau_analyzer::tautau_analyzer(const char* file1, const char* file2, string isMC, string sampleName)
{
...
tautau_analyzer::~tautau_analyzer()
{
  //std::cout<<"tautau_analyzer destructor called"<<std::endl;
  if (!fChain) return;
...
Int_t tautau_analyzer::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t tautau_analyzer::LoadTree(Long64_t entry)
{
...
void tautau_analyzer::Init(TChain *tree, string _isMC_ , string sampleName)
{
...
void tautau_analyzer::Show(Long64_t entry)
{
...
Int_t tautau_analyzer::Cut(Long64_t entry)
{
...
void tautau_analyzer::setMyEleTau(int tau1Index, int tau2Index){
  Tau1Index=tau1Index; Tau2Index=tau2Index;
  my_tau1P4.SetPtEtaPhiE(tau_Pt->at(tau1Index),tau_Eta->at(tau1Index)
			 ,tau_Phi->at(tau1Index), tau_Energy->at(tau1Index)
			 );
  my_tau2P4.SetPtEtaPhiE(tau_Pt->at(tau2Index),tau_Eta->at(tau2Index)
			 ,tau_Phi->at(tau2Index), tau_Energy->at(tau2Index)
			 );
  //cout<<"tau 1 : "<<my_tau1P4.Pt()<<" tau 2 : "<<my_tau2P4.Pt()<<endl;
  my_genmatching_l1 = myGenMaching(Tau1Index);
  my_genmatching_l2 = myGenMaching(Tau2Index);
  tau1dm  = tau_DecayMode->at(tau1Index);
  tau2dm  = tau_DecayMode->at(tau2Index);
  ...
  uncorrectedMetPlusTau=uncorrected_met+my_tau1P4+my_tau2P4;
  
  /// ES corrections
  //my_eleP4 = my_eleP4*(eleCalibE->at(eleIndex)/my_eleP4.E());
  if(is_MC){
    if (my_genmatching_l2>=5 && tau2dm==0) my_tau2P4=my_tau2P4*1.007;
    else if (my_genmatching_l2>=5 && tau2dm==1) my_tau2P4=my_tau2P4*0.998;
    else if (my_genmatching_l2>=5 && tau2dm==10) my_tau2P4=my_tau2P4*1.001;
    if (  (my_genmatching_l2==1 || my_genmatching_l2==3) && tau2dm==0 )
      my_tau2P4=my_tau2P4*1.003;
    else if ( (my_genmatching_l2==1 || my_genmatching_l2==3) && tau2dm==1)
      my_tau2P4=my_tau2P4*1.036;
  }
  if(is_MC){
    if (my_genmatching_l1>=5 && tau1dm==0) my_tau1P4=my_tau1P4*1.007;
    else if (my_genmatching_l1>=5 && tau1dm==1) my_tau1P4=my_tau1P4*0.998;
    else if (my_genmatching_l1>=5 && tau1dm==10) my_tau1P4=my_tau1P4*1.001;
    if (  (my_genmatching_l1==1 || my_genmatching_l1==3) && tau1dm==0 )
      my_tau1P4=my_tau1P4*1.003;
    else if ( (my_genmatching_l1==1 || my_genmatching_l1==3) && tau1dm==1)
      my_tau1P4=my_tau1P4*1.036;
  }
