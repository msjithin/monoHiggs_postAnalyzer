...
int main(int argc, char** argv) {
   ...
   TH1F* h0_mvis_iso = new TH1F ("h0_mvis_iso","h0_mvis_iso",35,0,350); h0_mvis_iso->Sumw2();
   TH1F* h0_mvis_anti = new TH1F ("h0_mvis_anti","h0_mvis_anti",35,0,350); h0_mvis_anti->Sumw2();
   ...
   TH1F* h0_tau1pt_iso = new TH1F ("h0_tau1pt_iso","h0_tau1pt_iso",30,0,150); h0_tau1pt_iso->Sumw2();
   TH1F* h0_tau1pt_anti = new TH1F ("h0_tau1pt_anti","h0_tau1pt_anti",30,0,150); h0_tau1pt_anti->Sumw2();

   TH1F* h0_pth_iso = new TH1F ("h0_pth_iso","h0_pth_iso",40,0,200); h0_pth_iso->Sumw2();
   TH1F* h0_pth_anti = new TH1F ("h0_pth_anti","h0_pth_anti",40,0,200); h0_pth_anti->Sumw2();
   ...
   for (Int_t i = 0; i < nentries_wtn; i++) {
	...
	//************************* Fill histograms **********************
	   float weight2=1.0;
	     ...
	     if (isL or isT){
		  ...
		  h0_mvis_iso->Fill((mytau1+mytau2).M(),aweight*weight2);
               if (antiisoRegion && iso2 && q_1*q_2<0)
                  h0_mvis_anti->Fill((mytau1+mytau2).M(),aweight*weight2*FF);

               if (signalRegion && iso2 && q_1*q_2<0)
                  h0_pth_iso->Fill((mytau1+mytau2+mymet).Pt(),aweight*weight2);
               if (antiisoRegion && iso2 && q_1*q_2<0)
                  h0_pth_anti->Fill((mytau1+mytau2+mymet).Pt(),aweight*weight2*FF);

               if (signalRegion && iso2 && q_1*q_2<0)
                  h0_tau1pt_iso->Fill(mytau1.Pt(),aweight*weight2);
               if (antiisoRegion && iso2 && q_1*q_2<0)
                  h0_tau1pt_anti->Fill(mytau1.Pt(),aweight*weight2*FF);

               if (signalRegion && iso2 && q_1*q_2<0)
                  h0_tau2pt_iso->Fill(mytau2.Pt(),aweight*weight2);
               if (antiisoRegion && iso2 && q_1*q_2<0)
                  h0_tau2pt_anti->Fill(mytau2.Pt(),aweight*weight2*FF);
    ...
    TDirectory *d0_mvis_iso =fout->mkdir("tt_mvis_iso");
    d0_mvis_iso->cd();
    h0_mvis_iso->SetName(name.c_str());
    h0_mvis_iso->Write();

    TDirectory *d0_mvis_anti =fout->mkdir("tt_mvis_anti");
    d0_mvis_anti->cd();
    h0_mvis_anti->SetName(name.c_str());
    h0_mvis_anti->Write();

    TDirectory *d0_met_iso =fout->mkdir("tt_met_iso");
    d0_met_iso->cd();
    h0_met_iso->SetName(name.c_str());
    h0_met_iso->Write();

    TDirectory *d0_met_anti =fout->mkdir("tt_met_anti");
    d0_met_anti->cd();
    ...
    TDirectory *d0_tau2pt_anti =fout->mkdir("tt_tau2pt_anti");
    d0_tau2pt_anti->cd();
    h0_tau2pt_anti->SetName(name.c_str());
    h0_tau2pt_anti->Write();

    TDirectory *d0_tau1pt_iso =fout->mkdir("tt_tau1pt_iso");
    d0_tau1pt_iso->cd();
    h0_tau1pt_iso->SetName(name.c_str());
    h0_tau1pt_iso->Write();

    TDirectory *d0_tau1pt_anti =fout->mkdir("tt_tau1pt_anti");
    d0_tau1pt_anti->cd();
    h0_tau1pt_anti->SetName(name.c_str());
    h0_tau1pt_anti->Write();

    TDirectory *d0_pth_iso =fout->mkdir("tt_pth_iso");
    d0_pth_iso->cd();
    h0_pth_iso->SetName(name.c_str());
    h0_pth_iso->Write();

    TDirectory *d0_pth_anti =fout->mkdir("tt_pth_anti");
    d0_pth_anti->cd();
    h0_pth_anti->SetName(name.c_str());
    h0_pth_anti->Write();
