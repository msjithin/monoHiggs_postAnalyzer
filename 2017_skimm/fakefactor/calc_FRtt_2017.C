...
//$ ./rootcom calc_FRtt_2017 analyze
//
...
//
#define calc_FRtt_2017_cxx
#include "calc_FRtt_2017.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
...
int main(int argc, const char* argv[])
{
  ...
  calc_FRtt_2017 t(argv[1],argv[2], isMC);
...
void calc_FRtt_2017::Loop(Long64_t maxEvents, int reportEvery, string SampleName, string _isMC_)
{
  ...
  tauCand.clear();
  std::vector<int> tau2Cand;
  tau2Cand.clear();

  std::vector<int> tauCand_t;
  tauCand_t.clear();
  std::vector<int> tau2Cand_t;
  tau2Cand_t.clear();
   ...
   for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
     {
	  ...
	  if(metFilters==0) 
		  ...
		  )
		{
		  ...
		  if(tauCand_t.size() >0)
		    tau2Cand_t = getTau2Cand(20,2.3, tauCand_t[0]);
		  if(tauCand_t.size() >0 && tau2Cand_t.size()>0)
		    {
		      if( tau_Charge->at(tauCand_t[0]) * tau_Charge->at(tau2Cand_t[0]) >0 ) // same charge condition
			{
			  if(debug==true)std::cout<<"This works! line 207" <<endl; nGoodMuMuPassed+=event_weight;
			  double deltaR_tt = dR_tt(tauCand_t[0], tau2Cand_t[0]);
			  if(deltaR_tt > 0.5 )
			    { 
			      if(debug==true)std::cout<<"This works! line 188" <<endl;
				...
				{ nPassedBjetVeto+=event_weight;
				  ...
				  myTau_1.SetPtEtaPhiE(tau_Pt->at(tauCand_t[0])
						       ,tau_Eta->at(tauCand_t[0])
						       ,tau_Phi->at(tauCand_t[0])
						       ,tau_Energy->at(tauCand_t[0]));
				  TLorentzVector myTau_2;
				  myTau_2.SetPtEtaPhiE(tau_Pt->at(tau2Cand_t[0])
                                                       ,tau_Eta->at(tau2Cand_t[0])
                                                       ,tau_Phi->at(tau2Cand_t[0])
                                                       ,tau_Energy->at(tau2Cand_t[0]));
				  
				  double visMass_tt = VisMass_F(myTau_1, myTau_2);
				  h_mumu_invM->Fill(visMass_tt);
				  if( visMass_tt > 80.0 && visMass_tt< 100 ) 
				    {
					...
					if (tau_byMediumDeepTau2017v2p1VSjet->at(tau2Cand[0])==1  )
					  {
					    nTightTau+=event_weight;
					    h_tauPt_inc_t->Fill(tau_Pt->at(tau2Cand[0]), event_weight);
					    h_tauM_t->Fill(myTau_2.M());
					    if (tau_DecayMode->at(tauCand[0])==0)
					      h_tauPt_dm0_t->Fill(tau_Pt->at(tau2Cand[0]), event_weight);
					    if (tau_DecayMode->at(tauCand[0])==1)
					      h_tauPt_dm1_t->Fill(tau_Pt->at(tau2Cand[0]), event_weight);
					    if (tau_DecayMode->at(tauCand[0])==10)
					      h_tauPt_dm10_t->Fill(tau_Pt->at(tau2Cand[0]), event_weight);
					    if (tau_DecayMode->at(tauCand[0])==11)
					      h_tauPt_dm11_t->Fill(tau_Pt->at(tau2Cand[0]), event_weight);
...
void calc_FRtt_2017::BookHistos(const char* file2)
{
...
void calc_FRtt_2017::fillHistos(int histoNumber, double event_weight, int higgs_Index)
{
...
std::vector<int> calc_FRtt_2017::getMuCand_1(double muPtCut, double muEtaCut){
...
std::vector<int> calc_FRtt_2017::getMuCand_2(double muPtCut, double muEtaCut, int skip_i){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over muons
  for(int iMu=0;iMu<nMu;iMu++)
...
std::vector<int> calc_FRtt_2017::getTauCand(double tauPtCut, double tauEtaCut){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
...
std::vector<int> calc_FRtt_2017::getTau2Cand(double tauPtCut, double tauEtaCut, int tau1Index){
  std::vector<int> tmpCand;
  tmpCand.clear();
  //Loop over taus      
  for(int iTau=0;iTau<nTau;iTau++)
...
bool calc_FRtt_2017::thirdLeptonVeto(int muon_1, int muon_2)
{
  //  std::vector<int> tmpCand;
  //tmpCand.clear();
  int tmpCand = 0;
...
double calc_FRtt_2017::dR(int mu_1_index, int mu_2_index )
{
...
double calc_FRtt_2017::dR_tt(int tau_1_index, int tau_2_index )
{
  double deltaeta = abs(tau_Eta->at(tau_1_index) - tau_Eta->at(tau_2_index));
  double t_1_Phi = tau_Phi->at(tau_1_index);
  double t_2_Phi = tau_Phi->at(tau_2_index);

  double deltaphi = DeltaPhi(t_1_Phi, t_2_Phi);
  double deltar = sqrt(deltaeta*deltaeta + deltaphi*deltaphi);
  return deltar;
}

double calc_FRtt_2017::dR_muTau(int mu_2_index, int tau_index )
{
...
double calc_FRtt_2017::DeltaPhi(double phi1, double phi2)
...
float calc_FRtt_2017::TMass_F(float LepPt, float LepPhi , float met, float metPhi) {
    return sqrt(pow(LepPt + met, 2) - pow(LepPt* cos(LepPhi) + met * cos(metPhi), 2) - pow(LepPt * sin(LepPhi) + met * sin(metPhi), 2));
}

float calc_FRtt_2017::TotTMass_F(TLorentzVector a, TLorentzVector b, TLorentzVector met) {
  float totalTMass = (a + b+ met).M();
  return totalTMass;
}


float calc_FRtt_2017::VisMass_F(TLorentzVector a, TLorentzVector b){
  float visibleMass = (a + b).M();
  return visibleMass;
}

float calc_FRtt_2017::pTvecsum_F(float pt1, float pt2, float phi1, float phi2) {
  float pt_vecSum = sqrt( pow(pt1*cos(phi1) + pt2*cos(phi2), 2) + pow(pt1*sin(phi1) + pt2*sin(phi2), 2));
  return pt_vecSum;
}

bool calc_FRtt_2017::passBjetVeto()
{
  int tmpCand = 0;
  bool veto = true;
