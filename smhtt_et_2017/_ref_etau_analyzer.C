				   ...
				   //if ( MatchTriggerFilter(reco_ele[0], reco_tau[0]) )
				     {
				       ...
				       double applySf=1.0;
				       if(isMC)
					 applySf=  getScaleFactors( elePt->at(reco_ele[0]),
								    tau_Pt->at(reco_tau[0]),
				       ...
				       if( thirdLeptonVeto(reco_ele[0] , reco_tau[0]) < 0 )
					 {
					   ...
					   if( passBjetVeto(reco_ele[0] , reco_tau[0], -1) == true)
						 ...
						 //       std::setw(10) << tau_Pt->at(reco_tau[0]) << std::setw(10) << tau_Eta->at(reco_tau[0]) << std::setw(10) <<  tau_Phi->at(reco_tau[0]) <<
						 //       std::setw(10) << nEle   << std::setw(10) << nTau    << std::setw(10) << nMu <<
						 //       std::setw(10) << nMC    << std::setw(10) << genMatch2_ << 
						 //       std::setw(10) << diEleVeto  << std::setw(10) << eVetoZtt  << std::setw(10) << mVetoZtt  << 
						 //       std::setw(10) << myGenMaching(reco_tau[0]) << std::setw(10) <<  myGenMaching1(reco_ele[0]) << std::setw(10) <<
						 //       "\n";
						 //   }
						 //cout<<"myGenMaching = "<< myGenMaching(reco_tau[0])<<endl;
						 ...
						 //   {
						 //     ptcd <<event<<"\t"<< myEle.Pt() <<"\t"<< myEle.Eta() <<"\t"<< myEle.Phi()
						 // 	  << "\t"      << dau2.Pt() <<"\t"<< tau_Eta->at(reco_tau[0])<<"\t"<< tau_Phi->at(reco_tau[0])<<"\t"
						 // 	  << eleid <<"\t" << (relEleIso) <<"\t"
						 ...
						 // 	  << deltaR  <<"\t"<< myGenMaching(reco_tau[0]) <<  "\t" << myGenMaching1(reco_ele[0]) <<  "\t"
						 // 	  <<"\n";
...
double etau_analyzer::getScaleFactors(  double elept, double taupt, double eleeta, double taueta, int taudm)
{
  bool debug=false;
  double rv_sf=1.0;
...
void etau_analyzer::makeMyPlot( string histNumber , int eleIndex, int ele2Index, int tauIndex, float event_weight){
  ...
  double mT_eleMet = TMass_F((elePt->at(ele_select)),(elePhi->at(ele_select)),pfMET,pfMETPhi  );
