			   ...
			   if (  eleCharge->at(reco_ele[0]) * tau_Charge->at(reco_tau[0]) < 0  ) 
			       ...
			       if (  MatchTriggerFilter(reco_ele[0], reco_tau[0]) )
				 {
				   ...
				   float mt=TMass_F(myEle.Pt(),myEle.Phi(),newMetCorr.Pt(), newMetCorr.Phi());
                                   float mvis=VisMass_F(myEle, dau2);
				   std::vector<int> jetCand;       jetCand.clear();
				   ...
				   double frac_qcd=0.24; double frac_w=0.75; double frac_tt=0.01;
				   int category=eventCategory(reco_ele[0],reco_tau[0]) ;
				   getFractions(category, mvis, frac_qcd, frac_w, frac_tt);
				   // printTabSeparated(
				   // 		     "category", category
				   // 		     ,"frac_qcd",frac_qcd
				   // 		     ,"frac_w", frac_w
...
void etau_analyzer::makeMyPlot( string histNumber , int eleIndex, int tauIndex, float event_weight){
  ...
  float my_fakefactor =1; // get_ff( dau2.Pt(), mt, mvis, jetCand.size(),
...
int etau_analyzer::eventCategory(int eleIndex, int tauIndex){
  ...
  dau1.SetPtEtaPhiE(elePt->at(eleIndex), eleEta->at(eleIndex), elePhi->at(eleIndex), eleE->at(eleIndex));
  initialMetTau=initialMet+dau2;
  applyESCorrections(dau1, dau2, eleIndex, tauIndex, dau1,dau2);
