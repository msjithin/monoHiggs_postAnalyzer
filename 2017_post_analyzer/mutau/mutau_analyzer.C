	   ...
	   if( HLTEleMuX>>21&1 == 1   )
	     {
	       ...
	       muCand = getMuCand(30,2.4);  ///// muons selected 
	       if( muCand.size() >0 ) 
		 { 
   ...
   for(int iMu=0;iMu<nMu;iMu++)
     {
       ...
       if( muIDbit->at(iMu)>>9&1==1 ) muonIso = true; // PFMedium isolation
...
double mutau_analyzer::getScaleFactors( int muIndex , int tauIndex, bool fakeBkg , bool isMC, bool debug)
{
  ...
  if(fakeBkg)
    rv_sf = sf_muID * sf_IsoEff * sf_muTrg * sf_tauidSF_m * sf_tauesSF * sf_fakeEle * sf_taufesSF ;
  else
    rv_sf = sf_muID * sf_IsoEff * sf_muTrg * sf_tauidSF_m * sf_tauidSF_vvvl * sf_tauesSF * sf_fakeEle * sf_taufesSF ;
...
void mutau_analyzer::fillHist( string histNumber , int muIndex, int tauIndex, float event_weight){
  ...
  plotFill("muEta_"+hNumber, muEta->at(muIndex), 30, -3, 3,  event_weight);
  ...
  plotFill("tauAntiEle_"+hNumber, tau_byVVVLooseDeepTau2017v2p1VSe->at(tauIndex), 8, -2, 2,  event_weight );
  ...
  plotFill("deltaR_"+hNumber, deltaR , 24, 0, 6,  event_weight);
...
void mutau_analyzer::fillHist( string histNumber , TLorentzVector muonP4, TLorentzVector tauP4, int muIndex, int tauIndex, float event_weight){
  ...
  plotFill("muEta_"+hNumber, muonP4.Eta(), 30, -3, 3,  event_weight);
