...
using namespace std;
Double_t Luminosity = 59700.0;//Lumi for inclusive
bool debugOn=false;

/////        add the name of histograms to be created     
//std::vector<string>  plotList = {"muPt_5", "muEta_5", "muPhi_5", "muDz_5", "muD0_5", "muonID_5", "relMuIso_5", "muCharge_5", "tauPt_5", "tauEta_5", "tauPhi_5", "tauIso_5", "tauDecayMode_5", "tauCharge_5", "tauAntiEle_5", "tauAntiMu_5" , "cutflow_n", "nEvents", "cutflow_Htt", "muPt_0", "muEta_0", "muPhi_0", "muDz_0", "muD0_0", "muonID_0", "relMuIso_0", "muCharge_0", "tauPt_0", "tauEta_0", "tauPhi_0", "tauIso_0", "tauDecayMode_0", "tauCharge_0", "tauAntiEle_0", "tauAntiMu_0"}; 
std::vector<string>  plotList = {"muPt_Plus_5","muEta_Plus_5","muPhi_Plus_5","muDz_Plus_5","muD0_Plus_5","muonID_Plus_5","relMuIso_Plus_5","muCharge_Plus_5","muPt_Minus_5","muEta_Minus_5","muPhi_Minus_5","muDz_Minus_5","muD0_Minus_5","muonID_Minus_5","relMuIso_Minus_5","muCharge_Minus_5","muMuMass_5"};
...
int main(int argc, char** argv)
{
  ...
  Double_t LOtoNNLO_DY = 6077.22/5322.0;
  Double_t LOtoNNLO_Wjets = 61526.7/52760.0; 
  if (sample=="ZL" || sample=="ZTT" || sample=="ZJ" || sample=="ZLL"){ xs=5322.0; weight=luminosity*xs/ngen;}
  else if (sample=="DY_LO" || sample=="DYJetsToLL"){ xs=LOtoNNLO_DY*5322.0; weight=luminosity*xs/ngen;}
  else if (sample=="DY1JetsToLL"){ xs=LOtoNNLO_DY*875.8; weight=luminosity*xs/ngen;}
  else if (sample=="DY2JetsToLL"){ xs=LOtoNNLO_DY*306.4; weight=luminosity*xs/ngen;}
  else if (sample=="DY3JetsToLL"){ xs=LOtoNNLO_DY*111.8; weight=luminosity*xs/ngen;}
  else if (sample=="DY4JetsToLL"){ xs=LOtoNNLO_DY*43.88; weight=luminosity*xs/ngen;}
  else if (sample=="DYJetsToLL_M10to50"){ xs=15820; weight=luminosity*xs/ngen;}
  ...
  else if (sample=="ZTTjet_inc"){ weight=3.621;}
  else if (sample=="ZTT1jet")   { weight=0.69923;}
  else if (sample=="ZTT2jet")   { weight=0.7965;}
  else if (sample=="ZTT3jet")   { weight=0.9825;}
  else if (sample=="ZTT4jet")   { weight=0.8209;}
  
  else if (sample=="WJetsToLNu_inc") {xs=LOtoNNLO_Wjets*52760.0;    weight=luminosity*xs/ngen;}
  else if (sample=="WJetsToLNu")     {xs=LOtoNNLO_Wjets*52760.0;    weight=luminosity*xs/ngen;}
  else if (sample=="W1JetsToLNu")    {xs=LOtoNNLO_Wjets*8103.0;     weight=luminosity*xs/ngen;}
  else if (sample=="W2JetsToLNu")    {xs=LOtoNNLO_Wjets*2786.0;     weight=luminosity*xs/ngen;}
  else if (sample=="W3JetsToLNu")    {xs=LOtoNNLO_Wjets*988.2;      weight=luminosity*xs/ngen;}
  else if (sample=="W4JetsToLNu")    {xs=LOtoNNLO_Wjets*543.8;      weight=luminosity*xs/ngen;}

  else if (sample=="WJets_inc"){    weight=51.7588;}
  else if (sample=="W1Jet")    {    weight=9.1066;}
  else if (sample=="W2Jet")    {    weight=7.1783;}
  else if (sample=="W3Jet")    {    weight=4.3482;}
  else if (sample=="W4Jet")    {    weight=3.5075;}
  ...
  else if (sample=="EWKWMinus" || sample=="EWKWMinus2Jets") {xs=23.09; weight=luminosity*xs/ngen;}
  else if (sample=="EWKWPlus" || sample=="EWKWPlus2Jets") {xs=29.57; weight=luminosity*xs/ngen;}
